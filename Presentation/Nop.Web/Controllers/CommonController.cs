  ...
  public partial class CommonController : BaseNopController
  {
    ...
    protected LanguageSelectorModel PrepareLanguageSelectorModel()
    {
      var availableLanguages =
        _cacheManager.Get(
          string.Format(
            ModelCacheEventConsumer.AVAILABLE_LANGUAGES_MODEL_KEY,
    ...
    protected CurrencySelectorModel PrepareCurrencySelectorModel()
    {
      var availableCurrencies =
        _cacheManager.Get(
          string.Format(
            ModelCacheEventConsumer.AVAILABLE_CURRENCIES_MODEL_KEY,
    ...
    protected int GetUnreadPrivateMessages()
    {
      ...
      {
        var privateMessages =
          _forumservice.GetAllPrivateMessages(
            _storeContext.CurrentStore.Id,
    ...
    public ActionResult SetLanguage(int langid, string returnUrl = "")
    {
      ...
      if (_localizationSettings.SeoFriendlyUrlsForLanguagesEnabled)
      {
        ...
        if (returnUrl.IsLocalizedUrl(applicationPath, true))
        {
          ...
          returnUrl =
            returnUrl.RemoveLanguageSeoCodeFromRawUrl(applicationPath);
        }
        returnUrl = returnUrl.AddLanguageSeoCodeToRawUrl(
          applicationPath, _workContext.WorkingLanguage);
      }
                                        ...
                                        string returnUrl = "")
    {
      var taxDisplayType =
        (TaxDisplayType)
        Enum.ToObject(typeof (TaxDisplayType), customerTaxType);
      ...
      if (unreadMessageCount > 0)
      {
        unreadMessage =
          string.Format(
            _localizationService.GetResource(
              "PrivateMessages.TotalUnread"), unreadMessageCount);

        //notifications here
        if (_forumSettings.ShowAlertForPM &&
            !customer.GetAttribute<bool>(
              ...
              _storeContext.CurrentStore.Id))
        {
          ...
          alertMessage =
            string.Format(
              _localizationService.GetResource(
                "PrivateMessages.YouHaveUnreadPM"), unreadMessageCount);
        }
      }
    ...
    public ActionResult Menu()
    {
      var model = new MenuModel()
        {
          RecentlyAddedProductsEnabled =
            _catalogSettings.RecentlyAddedProductsEnabled,
      ...
      var model = new InfoBlockModel()
        {
          ...
          RecentlyViewedProductsEnabled =
            _catalogSettings.RecentlyViewedProductsEnabled,
      ...
      var model = new ContactUsModel()
        {
          ...
          DisplayCaptcha =
            ...
            _captchaSettings.ShowOnContactUsPage
        };
      return View(model);
    }
                                      ...
                                      bool captchaValid)
    {
      ...
      if (_captchaSettings.Enabled &&
          _captchaSettings.ShowOnContactUsPage && !captchaValid)
      {
      ...
      if (ModelState.IsValid)
      {
        ...
        string subject =
          string.Format(
            _localizationService.GetResource("ContactUs.EmailSubject"),
            _storeContext.CurrentStore.Name);

        var emailAccount =
          _emailAccountService.GetEmailAccountById(
        ...
        if (_commonSettings.UseSystemEmailForContactUsForm)
        {
          ...
          body =
            string.Format(
              "<strong>From</strong>: {0} - {1}<br /><br />{2}",
    ...
    public ActionResult Sitemap()
    {
      ...
      if (_commonSettings.SitemapIncludeManufacturers)
      {
        ...
        model.Manufacturers =
          manufacturers.Select(x => x.ToModel()).ToList();
      }
      if (_commonSettings.SitemapIncludeProducts)
      {
        ...
        var products =
          _productService.SearchProducts(
            ...
            pageSize: 200);
        model.Products =
          products.Select(product => new ProductOverviewModel()
            {
              Id = product.Id,
    ...
    public ActionResult StoreThemeSelector()
    {
      ...
      var currentTheme =
        _themeProvider.GetThemeConfiguration(
          _themeContext.WorkingDesktopTheme);
      model.CurrentStoreTheme = new StoreThemeModel()
        {
          Name = currentTheme.ThemeName,
          Title = currentTheme.ThemeTitle
        };
      model.AvailableStoreThemes =
        _themeProvider.GetThemeConfigurations()
    ...
    public ActionResult ChangeDevice(bool dontUseMobileVersion)
    {
      _genericAttributeService.SaveAttribute(
        ...
        dontUseMobileVersion, _storeContext.CurrentStore.Id);
    ...
    public ActionResult EuCookieLaw()
    {
      ...
      if (
        _workContext.CurrentCustomer.GetAttribute<bool>(
          "EuCookieLaw.Accepted", _storeContext.CurrentStore.Id))
        //already accepted
        return Content("");
    ...
    public ActionResult EuCookieLawAccept()
    {
      ...
      //save setting
      _genericAttributeService.SaveAttribute(
        _workContext.CurrentCustomer, "EuCookieLaw.Accepted", true,
        _storeContext.CurrentStore.Id);
    ...
    public ActionResult RobotsTextFile()
    {
      ...
      if (_localizationSettings.SeoFriendlyUrlsForLanguagesEnabled)
      {
              ...
              storeId: _storeContext.CurrentStore.Id))
        {
          foreach (var path in localizableDisallowPaths)
          {
            sb.AppendFormat("Disallow: {0}{1}", language.UniqueSeoCode,
                            path);
            sb.Append(newLine);
          }
        }
