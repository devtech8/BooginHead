...
if(c.maxDistance===true){var E=a("#"+c.maxDistanceID).val();n(E);}else{n();}}if(c.noForm===true){a(document).on("click."+u,"#"+c.formContainerDiv+" #submit",function(E){D(E);
  ...
  a(function () {
    google.maps.visualRefresh = true; var H; if (c.dataType === "kml") { H = "xml"; } else { H = c.dataType; }
    alert(dataLocation);a.ajax({type:"GET",url:c.dataLocation+(c.dataType==="jsonp"?(c.dataLocation.match(/\?/)?"&":"?")+"callback=?":""),dataType:H,jsonpCallback:(c.dataType==="jsonp"?c.jsonpCallback:null),beforeSend:function(){if(c.callbackBeforeSend){c.callbackBeforeSend.call(this);
